 ...
 * 2. explicit free list << with no footer alloc block and ordering by LIFO
...
void set_succ_pred_LIFO(void *bptr);
...
void *extend_heap(int size)
{
    ...
    if (isLastAlloc == ALLOC)  
        resize = PACKING_BIT(resize, 2); // this bit indicates preceding block whether alloc or not
    isLastAlloc = FREE;
    PUT_VALUE(HEADPTR(bptr), resize);
    PUT_VALUE(FOOTPTR(bptr), resize); 
...
{   
    ...
    {   
        ...
        set_succ_pred_LIFO(freeBptr);                            // reconnect entire merged block
    }
...
void set_succ_pred_LIFO(void *bptr)
{
    if (head != NULL)
    {   
        PUT_PTR(SUCCPTR(bptr), head);
        PUT_PTR(PREDPTR(head), bptr);
    }
    else
    {   
